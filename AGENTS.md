...
> **Nota sobre persistencia:** En esta etapa académica, los datos residen en **arreglos tipados en memoria** (`src/models/`). No hay conexión activa a base de datos relacional (aunque la configuración base está preparada en `src/config/db.ts`).
...
├── .env                       # Variables de entorno locales (puerto, JWT secret, DB, URL UF)
...
└── src/
    ...
    ├── config/
    │   └── db.ts              # Placeholder de configuración de BD para fase futura
    ├── types/
    ...
    ├── models/
    │   ├── usuario.model.ts   # CRUD en memoria para usuarios (IUsuario[])
    │   └── proyecto.model.ts  # CRUD en memoria para proyectos (IProyecto[])
    ├── middlewares/
...
Como los modelos almacenan los datos en memoria:
1. Acceder a `http://localhost:3000/registro`.
2. Registrar un usuario de prueba (ej. Correo: `admin@test.cl`, Clave: `123456`).
3. Iniciar sesión en `http://localhost:3000/login`.
4. El sistema redirige automáticamente a `/proyectos`, permitiendo crear, editar, visualizar y eliminar proyectos.
